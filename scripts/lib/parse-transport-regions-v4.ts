...
/**
 * V4 transport parser:
 * - uses extracted states when present
 * - falls back to explicit region-state mapping when the page structure does not surface them clearly
 * - still keeps MSA extraction conservative
 */
export function enrichTransportationWithRegionsV4(
  html: string,
  ...
  for (const section of sections) {
    ...
    if (extractedStates.length === 0) warnings.push(`transport v4 used fallback state map for region ${section.title}`);
    if (msas.length === 0) warnings.push(`transport v4 extracted no MSAs for region ${section.title}`);
  }

  if (Object.keys(regions).length === 0) {
    warnings.push("transport v4 extracted no region blocks");
  }
