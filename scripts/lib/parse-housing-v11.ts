...
import { extractAdjacentHousingOverrideRowsV3 } from "./housing-adjacent-rows-v3";
import { WIDE_ADJACENCY_POLICY } from "./adjacency-policy";
...
export function parseHousingArtifactV11(params: {
  ...
  for (const [state, html] of Object.entries(params.htmlByState)) {
    ...
    if (count >= 3) {
      ...
      warnings.push(`housing v11 coverage too low for ${state}`);
    }

    const deduped = dedupeCounties(countyNames);
    if (deduped.length > 0 && deduped.length < countyNames.length) {
      warnings.push(`housing v11 parser deduped repeated counties for ${state}`);
    }

    const countyCount = countHousingCounties(model, state);
    if (countyCount > 0 && countyCount < 3) {
      warnings.push(`housing v11 retained only ${countyCount} county rows for ${state}`);
    }
  }

  if (parsedStates < 40) {
    throw new Error(`Housing parser v11 incomplete: expected at least 40 states parsed, got ${parsedStates}`);
  }

  const adjacentOverrides = Object.entries(params.htmlByState).flatMap(([state, html]) =>
    extractAdjacentHousingOverrideRowsV3(html, WIDE_ADJACENCY_POLICY)
  ...
  const msaOverrides = adjacentOverrides.length > 0 ? adjacentOverrides : derivedOverrides;

  if (adjacentOverrides.length === 0 && derivedOverrides.length > 0) warnings.push("housing v11 used derived overrides because wide-adjacency extraction yielded none");
  if (msaOverrides.length === 0) warnings.push("housing v11 produced no MSA overrides");
  ...
  return {
    ...
    coverage: `state defaults derived from retained county rows across ${parsedStates} states; total counties retained=${countHousingCounties(model)}; wide_adjacent_overrides=${adjacentOverrides.length}; overrides=${msaOverrides.length}`,
    warnings,
