...
import { DEFAULT_ADJACENCY_POLICY_V2, type AdjacencyPolicyV2 } from "./adjacency-policy-v2";
import { looksLikeImplicitHousingStop } from "./implicit-stop-conditions";

export interface AdjacentHousingOverrideRowV4 {
  label: string;
  counties: string[];
  utility?: number;
  mortgage?: number;
...
export function extractAdjacentHousingOverrideRowsV4(
  html: string,
  policy: AdjacencyPolicyV2 = DEFAULT_ADJACENCY_POLICY_V2,
): AdjacentHousingOverrideRowV4[] {
  const rows = tableRows(html);
  const out: AdjacentHousingOverrideRowV4[] = [];
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    const record: AdjacentHousingOverrideRowV4 = {
      label: headerText,
      counties: [],
    };
    ...
    for (let j = i + 1; j < Math.min(rows.length, i + 1 + policy.max_forward_rows); j += 1) {
      ...
      if (looksLikeImplicitHousingStop(nextText) && nextRole !== "county_membership") break;
