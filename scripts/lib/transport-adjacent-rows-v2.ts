...
export interface AdjacentTransportMsaRowV2 {
  label: string;
  state?: string;
  counties: string[];
  costs?: [number, number];
...
export function extractAdjacentTransportMsaRowsV2(html: string): AdjacentTransportMsaRowV2[] {
  const rows = tableRows(html);
  const out: AdjacentTransportMsaRowV2[] = [];
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    const record: AdjacentTransportMsaRowV2 = {
      label: headerText,
      state: extractState(headerText),
      counties: [],
    };

    for (let j = i + 1; j < Math.min(rows.length, i + 7); j += 1) {
      const nextRole = classifyTransportRow(rows[j]);
      const nextText = parseCells(rows[j], "both").join(" ").trim();
      if (!nextText) continue;
      if (nextRole === "msa_header" || nextRole === "region_header") break;
