...
export function enrichTransportationWithRegionsV11(
  html: string,
  ...
  for (const block of blocks) {
    ...
    const adjacentMsas = extractAdjacentTransportMsaRowsV3(block.body, WIDE_ADJACENCY_POLICY);
    ...
    if (regionalRows.length === 0) warnings.push(`transport v11 found no semantic regional cost row for ${block.header}`);
    if (adjacentMsas.length === 0) warnings.push(`transport v11 found no wide-adjacency MSA rows for ${block.header}`);
  }

  if (Object.keys(regions).length === 0) warnings.push("transport v11 extracted no semantic region blocks");
