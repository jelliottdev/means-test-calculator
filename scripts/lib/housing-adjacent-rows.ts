...
export interface AdjacentHousingOverrideRow {
  label: string;
  counties: string[];
  utility?: number;
  mortgage?: number;
...
export function extractAdjacentHousingOverrideRows(html: string): AdjacentHousingOverrideRow[] {
  const rows = tableRows(html);
  const out: AdjacentHousingOverrideRow[] = [];
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    const record: AdjacentHousingOverrideRow = {
      label: headerText,
      counties: [],
    };
