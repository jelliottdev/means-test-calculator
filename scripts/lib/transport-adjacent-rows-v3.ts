...
export interface AdjacentTransportMsaRowV3 {
  label: string;
  state?: string;
  counties: string[];
  costs?: [number, number];
...
export function extractAdjacentTransportMsaRowsV3(
  html: string,
  policy: AdjacencyPolicy = DEFAULT_ADJACENCY_POLICY,
): AdjacentTransportMsaRowV3[] {
  const rows = tableRows(html);
  const out: AdjacentTransportMsaRowV3[] = [];
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    const record: AdjacentTransportMsaRowV3 = {
      label: headerText,
