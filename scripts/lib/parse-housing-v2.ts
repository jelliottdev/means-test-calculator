 ...
 * V2 housing parser.
 *
 * Differences from the earlier skeleton:
 * - retains county rows in a generation-time model
 * - derives state defaults from retained county rows
 * - adds coverage warnings that reference county counts
 * - still avoids inventing MSA overrides when not clearly parsed
 */
export function parseHousingArtifactV2(params: {
  ...
  for (const [state, html] of Object.entries(params.htmlByState)) {
    ...
    if (count >= 3) {
      ...
      warnings.push(`housing v2 coverage too low for ${state}`);
    }

    const deduped = dedupeCounties(countyNames);
    if (deduped.length > 0 && deduped.length < countyNames.length) {
      warnings.push(`housing v2 parser deduped repeated counties for ${state}`);
    }

    const countyCount = countHousingCounties(model, state);
    if (countyCount > 0 && countyCount < 3) {
      warnings.push(`housing v2 retained only ${countyCount} county rows for ${state}`);
    }
  }

  if (parsedStates < 40) {
    throw new Error(`Housing parser v2 incomplete: expected at least 40 states parsed, got ${parsedStates}`);
  }
  ...
  return {
    ...
    coverage: `state defaults derived from retained county rows across ${parsedStates} states; total counties retained=${countHousingCounties(model)}`,
    warnings,
