...
export function parseHousingArtifactV5(params: {
  ...
  for (const [state, html] of Object.entries(params.htmlByState)) {
    ...
    if (count >= 3) {
      ...
      warnings.push(`housing v5 coverage too low for ${state}`);
    }

    const deduped = dedupeCounties(countyNames);
    if (deduped.length > 0 && deduped.length < countyNames.length) {
      warnings.push(`housing v5 parser deduped repeated counties for ${state}`);
    }

    const countyCount = countHousingCounties(model, state);
    if (countyCount > 0 && countyCount < 3) {
      warnings.push(`housing v5 retained only ${countyCount} county rows for ${state}`);
    }
  }

  if (parsedStates < 40) {
    throw new Error(`Housing parser v5 incomplete: expected at least 40 states parsed, got ${parsedStates}`);
  }
  ...
  if (directOverrides.length === 0 && derivedOverrides.length > 0) {
    warnings.push("housing v5 used derived overrides because direct MSA rows were not extracted");
  }
  if (msaOverrides.length === 0) {
    warnings.push("housing v5 produced no MSA overrides");
  }
  ...
  return {
    ...
    coverage: `state defaults derived from retained county rows across ${parsedStates} states; total counties retained=${countHousingCounties(model)}; overrides=${msaOverrides.length}`,
    warnings,
