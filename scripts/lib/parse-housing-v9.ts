...
import { extractAdjacentHousingOverrideRows } from "./housing-adjacent-rows";
...
export function parseHousingArtifactV9(params: {
  ...
  for (const [state, html] of Object.entries(params.htmlByState)) {
    ...
    if (count >= 3) {
      ...
      warnings.push(`housing v9 coverage too low for ${state}`);
    }

    const deduped = dedupeCounties(countyNames);
    if (deduped.length > 0 && deduped.length < countyNames.length) {
      warnings.push(`housing v9 parser deduped repeated counties for ${state}`);
    }

    const countyCount = countHousingCounties(model, state);
    if (countyCount > 0 && countyCount < 3) {
      warnings.push(`housing v9 retained only ${countyCount} county rows for ${state}`);
    }
  }

  if (parsedStates < 40) {
    throw new Error(`Housing parser v9 incomplete: expected at least 40 states parsed, got ${parsedStates}`);
  }

  const adjacentOverrides = Object.entries(params.htmlByState).flatMap(([state, html]) =>
    extractAdjacentHousingOverrideRows(html)
  ...
  if (adjacentOverrides.length === 0 && derivedOverrides.length > 0) warnings.push("housing v9 used derived overrides because adjacency-based extraction yielded none");
  if (msaOverrides.length === 0) warnings.push("housing v9 produced no MSA overrides");
  ...
  return {
    ...
    coverage: `state defaults derived from retained county rows across ${parsedStates} states; total counties retained=${countHousingCounties(model)}; adjacent_overrides=${adjacentOverrides.length}; overrides=${msaOverrides.length}`,
    warnings,
