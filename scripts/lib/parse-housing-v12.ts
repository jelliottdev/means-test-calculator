...
export function parseHousingArtifactV12(params: {
  ...
  for (const [state, html] of Object.entries(params.htmlByState)) {
    ...
    if (count >= 3) {
      ...
      warnings.push(`housing v12 coverage too low for ${state}`);
    }

    const deduped = dedupeCounties(countyNames);
    if (deduped.length > 0 && deduped.length < countyNames.length) {
      warnings.push(`housing v12 parser deduped repeated counties for ${state}`);
    }

    const countyCount = countHousingCounties(model, state);
    if (countyCount > 0 && countyCount < 3) {
      warnings.push(`housing v12 retained only ${countyCount} county rows for ${state}`);
    }
  }

  if (parsedStates < 40) {
    throw new Error(`Housing parser v12 incomplete: expected at least 40 states parsed, got ${parsedStates}`);
  }

  const adjacentOverrides = Object.entries(params.htmlByState).flatMap(([state, html]) =>
    extractAdjacentHousingOverrideRowsV3(html, LONG_RANGE_ADJACENCY_POLICY_V2)
  ...
  if (adjacentOverrides.length === 0 && derivedOverrides.length > 0) warnings.push("housing v12 used derived overrides because long-range adjacency extraction yielded none");
  if (msaOverrides.length === 0) warnings.push("housing v12 produced no MSA overrides");
  ...
  return {
    ...
    coverage: `state defaults derived from retained county rows across ${parsedStates} states; total counties retained=${countHousingCounties(model)}; long_range_adjacent_overrides=${adjacentOverrides.length}; overrides=${msaOverrides.length}`,
    warnings,
