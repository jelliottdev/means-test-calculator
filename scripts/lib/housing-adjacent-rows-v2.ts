...
export interface AdjacentHousingOverrideRowV2 {
  label: string;
  counties: string[];
  utility?: number;
  mortgage?: number;
...
export function extractAdjacentHousingOverrideRowsV2(html: string): AdjacentHousingOverrideRowV2[] {
  const rows = tableRows(html);
  const out: AdjacentHousingOverrideRowV2[] = [];
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    const record: AdjacentHousingOverrideRowV2 = {
      label: headerText,
      counties: [],
    };
