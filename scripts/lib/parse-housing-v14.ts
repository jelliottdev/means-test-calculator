...
import { extractAdjacentHousingOverrideRowsV5 } from "./housing-adjacent-rows-v5";
...
export function parseHousingArtifactV14(params: {
  ...
  for (const [state, html] of Object.entries(params.htmlByState)) {
    ...
    if (count >= 3) {
      ...
      warnings.push(`housing v14 coverage too low for ${state}`);
    }

    const deduped = dedupeCounties(countyNames);
    if (deduped.length > 0 && deduped.length < countyNames.length) {
      warnings.push(`housing v14 parser deduped repeated counties for ${state}`);
    }

    const countyCount = countHousingCounties(model, state);
    if (countyCount > 0 && countyCount < 3) {
      warnings.push(`housing v14 retained only ${countyCount} county rows for ${state}`);
    }
  }

  if (parsedStates < 40) {
    throw new Error(`Housing parser v14 incomplete: expected at least 40 states parsed, got ${parsedStates}`);
  }

  const adjacentOverrides = Object.entries(params.htmlByState).flatMap(([state, html]) =>
    extractAdjacentHousingOverrideRowsV5(html, BALANCED_ADJACENCY_POLICY_V3)
  ...
  if (adjacentOverrides.length === 0 && derivedOverrides.length > 0) warnings.push("housing v14 used derived overrides because balanced-adjacency extraction yielded none");
  if (msaOverrides.length === 0) warnings.push("housing v14 produced no MSA overrides");
