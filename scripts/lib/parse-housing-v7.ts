...
import { extractHousingMembershipRows } from "./housing-membership-semantics";
...
export function parseHousingArtifactV7(params: {
  ...
  for (const [state, html] of Object.entries(params.htmlByState)) {
    ...
    if (count >= 3) {
      ...
      warnings.push(`housing v7 coverage too low for ${state}`);
    }

    const deduped = dedupeCounties(countyNames);
    if (deduped.length > 0 && deduped.length < countyNames.length) {
      warnings.push(`housing v7 parser deduped repeated counties for ${state}`);
    }

    const countyCount = countHousingCounties(model, state);
    if (countyCount > 0 && countyCount < 3) {
      warnings.push(`housing v7 retained only ${countyCount} county rows for ${state}`);
    }
  }

  if (parsedStates < 40) {
    throw new Error(`Housing parser v7 incomplete: expected at least 40 states parsed, got ${parsedStates}`);
  }

  const semanticRowCount = Object.values(params.htmlByState).reduce((sum, html) => sum + extractHousingMsaLikeRows(html).length, 0);
  const membershipRows = Object.entries(params.htmlByState).flatMap(([state, html]) =>
    extractHousingMembershipRows(html)
      .filter((row) => row.counties.length > 0 && row.utility && row.mortgage)
      .map((row) => ({
        name: row.label,
        counties: { [state]: row.counties },
        utility: scaleHousehold(row.utility!),
        mortgage: scaleHousehold(row.mortgage!),
      }))
  );
  ...
  const msaOverrides = membershipRows.length > 0
    ? membershipRows
    : directOverrides.length > 0
      ? directOverrides
      : derivedOverrides;

  if (semanticRowCount === 0) warnings.push("housing v7 found no semantic MSA-like rows");
  if (membershipRows.length === 0 && directOverrides.length > 0) warnings.push("housing v7 used direct rows because membership semantics were empty");
  if (membershipRows.length === 0 && directOverrides.length === 0 && derivedOverrides.length > 0) warnings.push("housing v7 used derived overrides because semantic/direct rows did not yield overrides");
  if (msaOverrides.length === 0) warnings.push("housing v7 produced no MSA overrides");
  ...
  return {
    ...
    coverage: `state defaults derived from retained county rows across ${parsedStates} states; total counties retained=${countHousingCounties(model)}; semantic_msa_rows=${semanticRowCount}; membership_rows=${membershipRows.length}; overrides=${msaOverrides.length}`,
    warnings,
