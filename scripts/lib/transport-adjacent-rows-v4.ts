...
export interface AdjacentTransportMsaRowV4 {
  label: string;
  state?: string;
  counties: string[];
  costs?: [number, number];
...
export function extractAdjacentTransportMsaRowsV4(
  html: string,
  policy: AdjacencyPolicyV2 = DEFAULT_ADJACENCY_POLICY_V2,
): AdjacentTransportMsaRowV4[] {
  const rows = tableRows(html);
  const out: AdjacentTransportMsaRowV4[] = [];
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    const record: AdjacentTransportMsaRowV4 = {
      label: headerText,
    ...
    for (let j = i + 1; j < Math.min(rows.length, i + 1 + policy.max_forward_rows); j += 1) {
      ...
      if (looksLikeImplicitTransportStop(nextText) && nextRole !== "county_membership") break;
